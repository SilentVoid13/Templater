...
export class TemplaterSettingTab extends PluginSettingTab {
	...
	display(): void {
        ...
        const system_commands_details: HTMLDetailsElement = containerEl.createEl("details");
        system_commands_details.createEl("summary", { text: "System Commands" });

		new Setting(system_commands_details)
		...
		if (this.plugin.settings.enable_system_commands) {
			...
			this.plugin.settings.templates_pairs.forEach((template_pair) => {
				const div = system_commands_details.createEl('div');
				div.addClass("templater_div");

				const title = system_commands_details.createEl('h4', {
					text: 'User Function n°' + i,
				});
				title.addClass("templater_title");

				const setting = new Setting(system_commands_details)
				...
				div.appendChild(system_commands_details.lastChild);

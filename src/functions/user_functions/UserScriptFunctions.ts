...
export class UserScriptFunctions implements IGenerateObject {
    ...
    async generate_user_script_functions(config: RunningConfig): Promise<Map<string, Function>> {
        ...
        for (let file of files) {
            if (file.extension.toLowerCase() === "js") {
                await this.load_user_script_function(config, file, user_script_functions);
            }
        }
        return user_script_functions;
    }

    async load_user_script_function(config: RunningConfig, file: TFile, user_script_functions: Map<string, Function>): Promise<void> {
    ...
    async generate_object(config: RunningConfig): Promise<{}> {
        const user_script_functions = await this.generate_user_script_functions(config);

...
export async function processAsync(app: App, file: TFile, fn: (content: string) => Promise<string | void>): Promise<void> {
    let isProcessed = false;
    const RETRY_DELAY_IN_MILLISECONDS = 100;
    const TIMEOUT_IN_MILLISECONDS = 5000;
    const startTime = performance.now();
    while (true) {
        const oldContent = await app.vault.exists(file.path) ? await app.vault.read(file) : '';
